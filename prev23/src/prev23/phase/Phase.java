...
public abstract class Phase implements AutoCloseable {
	...
	protected Phase(String phaseName) {
		String loggedPhase = prev23.Compiler.cmdLineArgValue("--logged-phase");
		if ((loggedPhase != null) && loggedPhase.matches(phaseName + "|all")) {
			// Prepare the name of the xml file.
			String xmlFileName = prev23.Compiler.cmdLineArgValue("--xml");
			if (xmlFileName == null) {
				xmlFileName = prev23.Compiler.cmdLineArgValue("--src-file-name").replaceFirst("\\.[^./]*$", "") + "."
						+ phaseName + ".xml";
			}

			// Prepare the name of the supporting xsl file.
			String xslDirName = prev23.Compiler.cmdLineArgValue("--xsl");
			if (xslDirName == null) {
				xslDirName = "";
			}
